...
costCenterSchema.pre("validate", function calculateAvailable(next) {
  this.availableAmount = Number(this.annualBudget || 0) - Number(this.executedAmount || 0);
  next();
});
...
costCenterSchema.pre("findOneAndUpdate", async function calculateAvailableOnUpdate(next) {
  ...
  if (payload.annualBudget !== undefined || payload.executedAmount !== undefined) {
    ...
    payload.availableAmount = Number(annualBudget || 0) - Number(executedAmount || 0);
