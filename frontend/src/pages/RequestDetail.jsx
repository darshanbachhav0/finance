...
const emptyRelated = { accountsPayable: [], journalEntries: [], paymentBatches: [], reconciliation: null, audit: [], budgetPreview: { status: "PENDING_VALIDATION", lines: [] }, procurementReadiness: null };
...
export default function RequestDetail() {
  ...
  async function runAction(type, comments = "") {
    ...
    try {
      ...
      notify({ approve: "Electronic approval recorded.", observe: "Request observed.", return: "Request returned for correction.", reject: "Request rejected.", budget: "Budget control completed.", order: "Purchase or Service Order issued from approved request data.", close: "Request closed.", void: "Request annulled and any reservation was released." }[type]);
      setConfirm(null);
      await load();
  ...
  return <section>
    ...
    {request && <>
      ...
      <div className="request-detail-layout"><div className="request-detail-main">
        ...
        {related.procurementReadiness && <div className="workspace-panel detail-section procurement-readiness"><div className="section-heading"><div><h3>{t("Procurement Readiness")}</h3><p>{t("Derived from the existing approval, Budget, Supplier, quotation, and document controls.")}</p></div><StatusBadge status={!related.procurementReadiness.applicable ? "NOT_APPLICABLE" : related.procurementReadiness.ready ? "READY" : "NOT_READY"} /></div><div className="readiness-gates"><div><span>{t("Request approval")}</span><StatusBadge status={related.procurementReadiness.approval?.complete ? "COMPLETE" : "PENDING"} /></div><div><span>{t("Budget commitment")}</span><StatusBadge status={related.procurementReadiness.budget?.status || "NO_BUDGET"} /></div><div><span>{t("Supplier homologation")}</span><StatusBadge status={related.procurementReadiness.supplier?.homologationStatus || "MISSING"} /></div><div><span>{t("PRV status")}</span><strong>{related.procurementReadiness.supplier?.supplierCode || t("PRV missing")}</strong></div><div><span>{t("Quotation requirements")}</span><StatusBadge status={related.procurementReadiness.quotations?.complete ? "COMPLETE" : "INCOMPLETE"} /></div><div><span>{t("Required documents")}</span><StatusBadge status={related.procurementReadiness.documents?.complete ? "COMPLETE" : "INCOMPLETE"} /></div></div>{related.procurementReadiness.applicable && <div className="responsibility-map"><div><span>{t("Authorized Requester")}</span><strong>{request.requester?.name || request.solicitor?.name}</strong></div>{(request.approvalRouteSnapshot || []).filter((step) => step.required !== false).map((step) => <div key={step._id || `${step.sequence}-${step.approvalLevel}`}><span>{t(step.approvalLevel)}</span><strong>{step.completedBy?.name || t(step.role || "Pending")}</strong></div>)}<div><span>{t("Budget / Finance validation")}</span><strong>{related.procurementReadiness.budget?.complete ? t("Committed") : t("Pending")}</strong></div></div>}{related.procurementReadiness.issues?.length > 0 && related.procurementReadiness.applicable && <ul className="readiness-issues">{related.procurementReadiness.issues.map((issue) => <li key={issue.code}><XCircle size={15} /><div><strong>{t(issue.code)}</strong><span>{t(issue.message)}</span></div></li>)}</ul>}{request.purchaseOrder && <div className="order-summary"><ShoppingCart size={18} /><div><strong>{request.purchaseOrder.poNumber}</strong><span>{t(request.purchaseOrder.orderKind === "SERVICE" ? "Service Order" : "Purchase Order")} · {request.purchaseOrder.supplierCodeSnapshot || request.supplier?.supplierCode || "-"}</span></div><StatusBadge status={request.purchaseOrder.status} /></div>}</div>}
        ...
        <div className="workspace-panel detail-section"><div className="section-heading"><div><h3>{t("Financial control records")}</h3><p>{t("Budget, purchase order, CXP, journals, bank batches, payment, and reconciliation remain independently traceable.")}</p></div></div><dl className="detail-grid"><div><dt>{t("Budget status")}</dt><dd><StatusBadge status={request.budgetCommitment?.status || "NO_BUDGET"} /></dd></div><div><dt>{t("Budget amount")}</dt><dd>{request.budgetCommitment ? `PEN ${Number(request.budgetCommitment.totalAmount || 0).toFixed(2)}` : "-"}</dd></div><div><dt>{t("Purchase / Service Order")}</dt><dd>{request.purchaseOrder?.poNumber || "-"}</dd></div><div><dt>{t("Fiscal voucher")}</dt><dd>{request.fiscalData?.voucherType || request.fiscalData?.documentType ? `${request.fiscalData.voucherType || request.fiscalData.documentType} ${request.fiscalData.series}-${request.fiscalData.number}` : "-"}</dd></div><div><dt>{t("CXP status")}</dt><dd>{related.accountsPayable?.[0] ? <StatusBadge status={related.accountsPayable[0].status} /> : "-"}</dd></div><div><dt>{t("Payment Terms")}</dt><dd>{related.accountsPayable?.[0]?.paymentTermsSnapshot?.option ? `${t(related.accountsPayable[0].paymentTermsSnapshot.option)} · ${related.accountsPayable[0].paymentTermsSnapshot.days || 0} ${t("days")}` : "-"}</dd></div><div><dt>{t("Due date")}</dt><dd>{related.accountsPayable?.[0]?.dueDate ? new Date(related.accountsPayable[0].dueDate).toLocaleDateString() : "-"}</dd></div><div><dt>{t("Payment Destination Snapshot")}</dt><dd>{related.accountsPayable?.[0]?.bankAccountSnapshot?.bank ? `${related.accountsPayable[0].bankAccountSnapshot.bank} · ${related.accountsPayable[0].bankAccountSnapshot.sourceType || "SUPPLIER"}` : "-"}</dd></div><div><dt>{t("Bank batch")}</dt><dd>{related.paymentBatches?.[0]?.batchNumber || request.paymentBatch?.batchNumber || "-"}</dd></div><div><dt>{t("Payment operation")}</dt><dd>{request.payment?.operationNumber || "-"}</dd></div><div><dt>{t("Reconciliation")}</dt><dd>{related.reconciliation ? `${related.reconciliation.bankReference} / ${Number(related.reconciliation.difference || 0).toFixed(2)}` : "-"}</dd></div></dl>
      ...
      <aside className="request-detail-side">
        {(permissions.modifiable || permissions.canApprove || permissions.canCommitBudget || permissions.canIssueOrder || permissions.canClose || permissions.canVoid) && <div className="workspace-panel action-panel"><div className="section-heading"><div><h3>{t("Available actions")}</h3><p>{t("The backend revalidates permission, period, status, documents, supplier, fiscal, and budget controls.")}</p></div></div>
          ...
          {permissions.canIssueOrder && <button type="button" className="primary-button" onClick={() => setConfirm({ type: "order", title: "Issue the Purchase or Service Order?", description: "The order will use the approved request supplier, PRV, lines, currency, and amount. Repeated requests return the existing order.", confirmLabel: "Issue order", details: [{ label: "Request", value: request.requestNumber }, { label: "Order type", value: related.procurementReadiness.orderKind }, { label: "Supplier / PRV", value: `${request.supplier?.legalName || request.supplier?.name} / ${request.supplier?.supplierCode || "-"}` }, { label: "Result", value: "An immutable OC reference and approved-data snapshot are created." }] })}><ShoppingCart size={16} /><span>{t("Issue order")}</span></button>}
