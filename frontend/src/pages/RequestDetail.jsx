            ...
            <div className="request-detail-main">
              ...
              <div className="workspace-panel detail-section">
                ...
                <DataTable controls={false} rows={request.attachments} columns={[
                  ...
                  { key: "download", label: "", sortable: false, render: (row) => <a className="icon-button" href={`${apiHost}${row.url}`} target="_blank" rel="noreferrer" title={t("Preview or download")}><Download size={16} /></a> }
                ]} />
              ...
              {(request.bankFile?.fileName || request.rendition?.submittedAt) && (
                <div className="workspace-panel detail-section">
                  ...
                  <dl className="detail-grid">
                    {request.bankFile?.fileName && <><div><dt>{t("Bank file")}</dt><dd><a href={`${apiHost}${request.bankFile.url}`} target="_blank" rel="noreferrer">{request.bankFile.fileName}</a></dd></div><div><dt>{t("Generated")}</dt><dd>{new Date(request.bankFile.generatedAt).toLocaleString()}</dd></div><div><dt>{t("Generated by")}</dt><dd>{request.bankFile.generatedBy?.name || "-"}</dd></div></>}
